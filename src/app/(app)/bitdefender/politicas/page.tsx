...
import { listBitdefenderPolicies } from "@/lib/bitdefender/policies";
...
export default async function BitdefenderPoliciesPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
    ...
    return (
      <div className="space-y-6">
        ...
        <Alert>
          ...
          <AlertDescription>
            Cadastre uma conexão com o GravityZone antes de consultar políticas.{" "}
  ...
  let policies: Awaited<ReturnType<typeof listBitdefenderPolicies>> = [];
  let loadError: string | null = null;
  try {
    const config = await loadBitdefenderConnectionConfig(connection.id);
    policies = await listBitdefenderPolicies(config);
  } catch (err) {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Políticas de segurança cadastradas no GravityZone — atribua a um ou mais endpoints pelo ID.
        </p>
      </div>

      <BitdefenderConnectionPicker connections={connections} selectedId={connection.id} basePath="/bitdefender/politicas" />
      ...
      {loadError ? (
        ...
        <Card>
          <CardContent className="p-0">
            <PoliciesTable policies={policies} connectionId={connection.id} />
          </CardContent>
        </Card>
      )}
    </div>
