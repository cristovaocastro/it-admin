...
import { testSavedBitdefenderConnectionAction, deleteBitdefenderConnectionAction } from "@/lib/actions/bitdefender-connections-actions";
...
export function ConnectionActions({ connectionId, connectionName }: { connectionId: string; connectionName: string }) {
  ...
  function handleTest() {
    startTest(async () => {
      const result = await testSavedBitdefenderConnectionAction(connectionId);
  ...
  function handleDelete() {
    startDelete(async () => {
      const result = await deleteBitdefenderConnectionAction(connectionId);
      if (result?.error) toast.error(result.error);
      else {
        toast.success("Conexão removida.");
        router.push("/bitdefender/conexoes");
      }
    });
  }
      ...
      <AlertDialog>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Essa ação não afeta a conta GravityZone em si, apenas remove o cadastro da conexão neste painel. Não
              pode ser desfeita.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
