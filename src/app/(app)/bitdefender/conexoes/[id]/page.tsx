...
export default async function BitdefenderConnectionDetailPage({ params }: { params: Promise<{ id: string }> }) {
  await requireRole(["ADMIN"]);
  const { id } = await params;
  const connection = await db.bitdefenderConnection.findUnique({ where: { id } });
    ...
    <div className="mx-auto max-w-2xl space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{connection.apiUrl}</p>
