...
import { testSavedAdConnectionAction, deleteAdConnectionAction } from "@/lib/actions/ad-connections-actions";
...
export function ConnectionActions({ connectionId, connectionName }: { connectionId: string; connectionName: string }) {
  ...
  function handleTest() {
    startTest(async () => {
      const result = await testSavedAdConnectionAction(connectionId);
  ...
  function handleDelete() {
    startDelete(async () => {
      const result = await deleteAdConnectionAction(connectionId);
      if (result?.error) toast.error(result.error);
      else {
        toast.success("Conexão removida.");
        router.push("/ad/conexoes");
      }
    });
  }
      ...
      <AlertDialog>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Essa ação não afeta o Active Directory em si, apenas remove o cadastro da conexão neste painel. Não
              pode ser desfeita.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
