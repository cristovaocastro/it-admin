...
import { testSavedAwsConnectionAction, deleteAwsConnectionAction } from "@/lib/actions/aws-connections-actions";
...
export function ConnectionActions({ connectionId, connectionName }: { connectionId: string; connectionName: string }) {
  ...
  function handleTest() {
    startTest(async () => {
      const result = await testSavedAwsConnectionAction(connectionId);
  ...
  function handleDelete() {
    startDelete(async () => {
      const result = await deleteAwsConnectionAction(connectionId);
      if (result?.error) toast.error(result.error);
      else {
        toast.success("Conexão removida.");
        router.push("/aws/conexoes");
      }
    });
  }
      ...
      <AlertDialog>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Essa ação não afeta a conta AWS em si, apenas remove o cadastro da conexão neste painel. Não pode ser
              desfeita.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
