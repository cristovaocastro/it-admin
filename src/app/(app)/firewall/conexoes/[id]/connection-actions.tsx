...
import { testSavedFirewallConnectionAction, deleteFirewallConnectionAction } from "@/lib/actions/firewall-connections-actions";
...
export function ConnectionActions({ connectionId, connectionName }: { connectionId: string; connectionName: string }) {
  ...
  function handleTest() {
    startTest(async () => {
      const result = await testSavedFirewallConnectionAction(connectionId);
  ...
  function handleDelete() {
    startDelete(async () => {
      const result = await deleteFirewallConnectionAction(connectionId);
      if (result?.error) toast.error(result.error);
      else {
        toast.success("Conexão removida.");
        router.push("/firewall/conexoes");
      }
    });
  }
      ...
      <AlertDialog>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Essa ação não afeta o firewall em si, apenas remove o cadastro da conexão neste painel. Não pode ser
              desfeita.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
