...
export default async function FirewallConnectionDetailPage({ params }: { params: Promise<{ id: string }> }) {
  await requireRole(["ADMIN"]);
  const { id } = await params;
  const connection = await db.firewallConnection.findUnique({ where: { id } });
