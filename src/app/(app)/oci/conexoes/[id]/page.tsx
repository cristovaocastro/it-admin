...
export default async function OciConnectionDetailPage({ params }: { params: Promise<{ id: string }> }) {
  await requireRole(["ADMIN"]);
  const { id } = await params;
  const connection = await db.ociConnection.findUnique({ where: { id } });
    ...
    <div className="mx-auto max-w-2xl space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Regiões: {connection.regions.join(", ") || "—"}</p>
          <p className="text-sm text-muted-foreground">Compartments: {connection.compartments.length}</p>
